...
let
  ...
  dexConfig = pkgs.writeText "dex-config.yaml" ''
    ...
    storage:
      type: sqlite3
      config:
        file: /var/lib/private/dex/dex.db
    oauth2:
...
in
{
  ...
  config = lib.mkIf config.services.dex-wrapped.enable {
    # Disable the NixOS dex module's config generation - we manage our own
    services.dex.enable = false;

    # Create dex user and group since we're not using the module
    users.users.dex = {
      isSystemUser = true;
      group = "dex";
      home = "/var/lib/dex";
