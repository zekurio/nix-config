...
let
  ...
  groupedHosts = lib.foldl' (acc: name:
    let
      hostCfg = cfg.virtualHosts.${name};
      domain = hostCfg.domain or name;
      existing = acc.${domain} or { reverseProxies = []; extraConfigs = []; };
    in
      acc // {
        ${domain} = {
          reverseProxies = existing.reverseProxies ++ 
            (lib.optional (hostCfg.reverseProxy or null != null) hostCfg.reverseProxy);
          extraConfigs = existing.extraConfigs ++ 
